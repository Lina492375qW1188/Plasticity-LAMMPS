...
#include <math.h>
#include <stdlib.h>
#include "angle_cosine_plasticity.h"
#include "atom.h"
#include "neighbor.h"
#include "domain.h"
#include "comm.h"
...
AngleCosinePlasticity::AngleCosinePlasticity(LAMMPS *lmp) : Angle(lmp) {}

/* ---------------------------------------------------------------------- */

AngleCosinePlasticity::~AngleCosinePlasticity()
{
  if (allocated) {
    memory->destroy(setflag);
    memory->destroy(k);
    memory->destroy(mkT);  // release(T)
    memory->destroy(cy);   // release(T)
  }
}

/* ---------------------------------------------------------------------- */

void AngleCosinePlasticity::compute(int eflag, int vflag)
{
  int i1,i2,i3,n,type;
...
------------------------------------------------------------------------- */
  ...
  double *kT = new double[nanglelist];          //
  double *yT = new double[nanglelist];          //
...
------------------------------------------------------------------------- */
    if (c > cy[type] || *(iT+n) > 0){
        *(kT+n) = k[type]*mkT[type];       // mkT[type] (T)
        *(yT+n) = cy[type];                //  cy[type] (T)
        *(iT+n) = 1;
    }
    else {
        *(kT+n) = k[type];
...
/* ---------------------------------------------------------------------- */
  ...
  delete [] kT; // release memory (T)
  delete [] yT; // release memory (T)
  
}

/* ---------------------------------------------------------------------- */

void AngleCosinePlasticity::allocate()
{
  allocated = 1;
...
void AngleCosinePlasticity::coeff(int narg, char **arg)
{
...
double AngleCosinePlasticity::equilibrium_angle(int i)
{
  return MY_PI;
}
...
void AngleCosinePlasticity::write_restart(FILE *fp)
{
...
void AngleCosinePlasticity::read_restart(FILE *fp)
{
  allocate();
...
void AngleCosinePlasticity::write_data(FILE *fp)
{
...
double AngleCosinePlasticity::single(int type, int i1, int i2, int i3)
{
  double **x = atom->x;
...
------------------------------------------------------------------------- */
  ...
  double *yT = new double;
  if (c > cy[type] || *iT > 0){
      *kT = k[type]*mkT[type];  // mkT[type] (T)
      *yT = cy[type];           // cy (T)
      *iT = 1;
  }
  else {                        // for c <= cy && iT <= 0(T)        
      *kT = k[type];
      *yT = 1.0;
  }
